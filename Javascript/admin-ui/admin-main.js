...
function initPageScript(pageUrl) {
  ...
  if (pageUrl.includes('admin-appointment-report.php')) {
    ...
    if (typeof initAdminPerformanceReport === 'function') {
      ...
      const script = document.createElement('script');
      script.src = '../../Javascript/admin-ui/admin-subfolder/report-folder/admin-performance-reports.js';
      script.defer = true;
...
if (
  ...
  if (typeof initAdminAddNew === 'function') {
    initAdminAddNew(); // Already loaded, just run it
  } else {
    const script = document.createElement('script');
    script.src = '../../Javascript/admin-ui/admin-subfolder/shared-folder-add/admin-add-new.js';
    script.defer = true;
    script.onload = () => initAdminAddNew(); // Run after loading
    document.body.appendChild(script);
  }
}
